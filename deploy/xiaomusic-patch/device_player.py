...
class XiaoMusicDevice:
    ...
    async def _play_internal(self, name="", search_key="", allow_download=True):
        ...
        if not names:
            # [home-patch] 本地无歌时，优先走 MusicFree 插件聚合搜索在线直链播放（不下载落盘）
            if allow_download and await self._try_online_direct_play(
                search_key or name
            ):
                return
    ...
    async def _try_online_direct_play(self, keyword: str) -> bool:
        """[home-patch] 本地库无歌时，走 MusicFree 插件聚合搜索并在线直链播放（不下载落盘）

        Returns:
            bool: True 表示已成功发起在线播放；False 表示未找到/失败（交回原流程）
        """
        keyword = (keyword or "").strip()
        if not keyword:
            return False
        try:
            self.log.info(f"[home-patch] 本地未命中，尝试插件在线直链: {keyword}")
            res = await self.xiaomusic.online_music_service.search_top_one_play(
                did=self.did, search_key=keyword, name=keyword
            )
            ok = bool(res and isinstance(res, dict) and res.get("success"))
            self.log.info(
                f"[home-patch] 在线直链结果: {'成功' if ok else f'未找到({res})'}"
            )
            return ok
        except Exception as e:
            self.log.warning(f"[home-patch] 在线直链播放失败: {e}")
            return False
